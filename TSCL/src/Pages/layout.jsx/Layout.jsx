...
const Layout = ({ route }) => {
  ...
  const Menus = [
    ...
    { title: "Department", icon: <FaConnectdevelop />, to: "/department" },
    {
      title: "Locality",
      icon: <GrLocal />,
      submenu: true,
    ...
    { title: "Complaint", icon: <VscReport />, to: "/complaint" },
    { title: "Grievances", icon: <VscReport />, to: "/grievances" },
    { title: "Admin User", icon: <FaUser />, to: "/admin" },
    { title: "Public User", icon: <FaUser />, to: "/user" },
    { title: "Setting", icon: <IoMdSettings />, to: "/setting" },
    { title: "Logout", icon: <CiLogout />, to: "/logout" },
  ];
          ...
          <ul className="pt-2">
            {Menus.map((menu, index) => (
              <React.Fragment key={index}>
                <NavLink to={menu.to}>
                  <li
                    className={`text-white cursor-pointer text-md flex items-center gap-x-3 p-2 mt-1 pl-6 transition-all duration-75 hover:bg-sidebar-sl ${
                      location.pathname === menu.to
                        ? "bg-sidebar-sl transition-all duration-700"
                        : ""
                    }`}
                  >
                  ...
                  <ul>
                    {menu.submenuItems.map((submenuitem, subIndex) => (
                      <NavLink to={submenuitem.to} key={subIndex}>
                        <li
                          className={` cursor-pointer font-alegerya text-sm flex items-center gap-x-4 p-2 pl-20 hover:bg-gray-200 hover:text-black ${
